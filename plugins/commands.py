...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        if curr_time < 12:
            ...
            gtxt = "ɢᴏᴏᴅ ɴɪɢʜᴛ 👋"
        m=await message.reply_text("<i>ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ <b>ᴛʜᴇ 𝙼𝚁𝗔𝗞 𝗟𝗶𝗻𝗸𝗭𝘇𝘇 ʙᴏᴛ</b>.\nʜᴏᴘᴇ ʏᴏᴜ'ʀᴇ ᴅᴏɪɴɢ ᴡᴇʟʟ...</i>")
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            [
                            [
                                ...
                                InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @IamMrAK_bot
                            ]
                        ]
        )
    )
        ...
        if not await db.has_premium_access(user_id) and settings['is_shortlink']: #Don't change anything without my permission @IamMrAK_bot
            files_ = await get_file_details(file_id)
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : 𝙼𝚁𝗔𝗞 𝗟𝗶𝗻𝗸𝗭𝘇𝘇')
...
async def settutorial(bot, message):
    ...
    if len(message.command) == 1:
        ...
        return await message.reply("<b>ʏᴏᴜ ᴇɴᴛᴇʀᴇᴅ ɪɴᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ !\nᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ : /set_tutorial <code>https://t.me/MrAK_LinkZzz/5</code></b>")
